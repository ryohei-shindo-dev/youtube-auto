# PENDING_TASKS - 残タスク一覧

> 最終更新: 2026-03-15
> Codex / Claude Code 共有。各タスクは「状態 → 次のアクション → 関連ファイル」だけ書く。

---

## 1. TikTok 本番審査の通過対応

**状態**: TikTok 2回目も否認（2026-03-16）。API自動投稿はいったん断念。

**否認理由**: 個人利用・社内利用不可。Content Posting API は第三者ユーザー向けプラットフォーム用であり、自社アカウントへの自動投稿は想定用途外（ChatGPT評価 2026-03-16）。

**方針**: TikTok API 再申請は止める。手動投稿前提で最低限アカウントを育てる。YouTube / Instagram を優先。

**関連ファイル**: `tiktok_auth.py`, `auto_publish.py`, `docs/privacy.html`, `docs/terms.html`, `.env`

---

## 2. 5分動画（長尺）3本目の企画・制作

**状態**: 2本公開済みだが再生数ほぼゼロ（7回/5回、3/14公開）。ChatGPT評価（2026-03-16）: やめる必要はないが主戦場にしない。頻度を落として型を探る段階。
- 01: https://youtu.be/othACgQKmt8（7回再生）
- 02: https://youtu.be/tVxLD4myubY（5回再生）

**方針変更（2026-03-16）**:
- 頻度: 週1→2〜3週に1本
- テーマ: 感情だけ→「感情+具体テーマ+何が分かるか」
- 3本目テーマ第一候補: 「オルカンとS&P500で揺れる人へ｜乗り換えたくなるときに整理したいこと」
- 導線: 長尺公開前後に導線用Shortsを2〜3本作る

**実績（2026-03-16）**:
- 3本目「オルカンとS&P500で揺れる人へ」公開完了: https://youtube.com/shorts/MLQx3Ne_AmM
- ElevenLabs v3 に切替（読み精度・声質安定性が大幅改善）
- 台本はChatGPTレビュー反映済み

**次のアクション**:
1. 導線用Shorts台本を作成（3本目への誘導）
2. 3本で型を比較判断（2〜3週間後）
3. 4本目のテーマ検討

**関連ファイル**: `long_video_builder_02.py`, `long_video/`, `long_voice_gen.py`

---

## 3. 英語版チャンネルの立ち上げ

**状態**: 検討中。台本20本・hook集・TTS原稿は準備済み。

**次のアクション**:
1. チャンネル名を最終決定（候補: Stay Invested / Hold Steady / Still Holding）
2. YouTube チャンネル作成 → ElevenLabs 英語音声テスト → 20本生成・投稿

**関連ファイル**: `memory/english_channel.md`, `memory/english_tts_scripts.md`

---

## 4. 新旧プロンプトA/Bテスト観察

**状態**: 新プロンプト14本をキューに交互配置済み（2026-03-14）。旧2本:新1本の比率で投稿される。ChatGPTアプローチレビュー（2026-03-14）で「観察モードに入るべき」と評価 — 重大バグ以外は手を入れない期間。

**次のアクション**:
1. 新プロンプト動画の初速・再生数・いいね率を旧動画と比較（重大バグ以外コード変更しない）
2. 2〜3週間後（2026-03-28頃）に効果判定（新プロンプトが優位なら次バッチも新プロンプトで生成）
3. 効果が出なければプロンプトを再調整

**関連ファイル**: `script_gen.py`, `publish_queue.json`, `analytics_collect.py`

---

## 5. Shorts 在庫管理

**状態**: キュー62本（31日分）。整理完了（2026-03-19）。

**実績（2026-03-19）**:
- 84本→62本に絞り込み（ChatGPT評価で22本除外: コンセプト不一致10/やめた後悔重複4/配当重複3/3年目重複3/テーマ弱2）
- 7:00/12:00出し分け: 奇数=数字・差分・損失系、偶数=共感・比較・継続系で交互配置
- 同系統の連投を避けた順序に並べ替え済み

**次のアクション（3/28以降）**:
1. A/Bテスト結果を踏まえて補充本数を確定
2. 補充12〜16本を生成（batch_api_gen.py）
   - 普通の自分でも大丈夫系 / こつこつ継続肯定系 / 制度・数字の静かな整理系
   - 暴落系・売りたい系・含み損系・高配当系は増やさない

**関連ファイル**: `batch_gen.py`, `stock_scorer.py`, `publish_queue.json`

---

## 6. note 運用

**状態**: 記事フォーマット統一完了 + 16本補充中（2026-03-15）

**実績（2026-03-15）**:
- 全32本の見出しフォーマット修正完了（■→H3見出し）
- 追加16本の記事・画像生成完了（比較・目移り・正解不安・出口不安テーマ）
- 16本を3/19〜3/27に予約投稿中（3/19〜3/25は1日2本、3/26〜3/27は1日1本）
- マガジン名「静かに持ち続ける人へ」→「長期投資の夜に読むメモ」に変更予定
- noteプレミアム: 4〜6月は継続、6月末に再判定（ChatGPT評価）

**実績（2026-03-16追加）**:
- 16本の予約投稿にヘッダー画像を追加完了（新素材で再生成、予約状態維持）
- note_image_gen.py: 写真選択をローテーション→ランダムに変更（同じ画像が繰り返し使われる問題を修正）
- 記事管理ページURL: `note.com/notes`、セレクタ: `a.o-articleList__link`（DevTools確認済み）

**実績（2026-03-16追加②）**:
- マガジン名決定:「静かに持ち続ける人へ」→「こつこつ積み立てを続ける人の読みもの」
- マガジンヘッダー画像を再生成（recovery43背景、チャンネル名なし、垂直センター）
  - 保存先: `note_images/magazine_header_new.png`

**実績（2026-03-16追加③）**:
- こつこつ系コンテンツ戦略を導入（ChatGPT評価: ブランドの芯に据える）
  - script_gen.py: 継続モチベ系の代替感情曲線（違和感→整理→肯定）をテンプレートに追加
  - topics.json: 継続モチベ系に10本のこつこつ系トピック追加（17→27本）
  - hooks.json: 違和感型に「静かな違和感」hook 8本追加（8→16本）
- こつこつ系 note 記事 8本を生成+予約投稿完了（note_add_22〜29、3/26〜3/30）
- 縦型没入レイアウト統一+274枚素材で117本の動画再生成完了

**実績（2026-03-19）**:
- SEOリライト Phase 1 完了: タイトル+見出し+冒頭文リライト10本 → noteに反映済み
- SEOリライト Phase 2 完了: H2見出しリライト46本 → noteに反映済み
- 運用手順書: `docs/note-rewrite-runbook.md`

**実績（2026-03-19 SEOレビュー）**:
- ChatGPT SEOレビュー実施（テーマ選定8.5/タイトルSEO6.5/冒頭文7.5/見出し7/キーワード戦略7/SNS導線7.5）
- タイトル修正5本（検索キーワード強化: 記事23/30/49/50/53）→ noteに反映済み
- ハッシュタグ全56本再設定（共通3タグ+記事別1-2タグ）→ noteに反映済み
- ヘッダー画像5本再生成+noteアップロード完了（タイトル変更に合わせて）
- 内部リンクテキスト更新（旧タイトル→新タイトル）
- note_manifest.json 導入（sheet_noを唯一の主キーとした一元管理、ChatGPT設計レビュー済み）
- note_article_updater.py 作成（画像再生成+アップロード汎用スクリプト）
- note_update_seo.py 作成（タイトル・タグ更新スクリプト）

**次のアクション**:
1. カバーできていないキーワードの新記事6本（新NISA含み損 / 何を買う / オルカンvsS&P500どっち / 月いくら / 暴落時どうする / iDeCo vs 新NISA）
2. 構成を4型に分散（検索解説型 / 比較整理型 / 不安共感型 / こつこつ肯定型）
3. manifest基準にコードを移行（フェーズ2: 新規記事はsheet_XXX方式）

**関連ファイル**: `note_publish.py`, `note_gen.py`, `note_image_gen.py`, `note_articles/`, `note_images/`, `note_manifest.json`, `note_article_updater.py`, `note_update_seo.py`, `build_manifest.py`, `docs/note-rewrite-runbook.md`

---

## 8. Threads自動投稿（MVPテスト）

**状態**: API認証完了、自動投稿稼働開始（2026-03-19）。2週間テスト中。

**実績（2026-03-19）**:
- Threads専用Meta Appを新規作成（既存Instagram用Appではコールバック保存エラーが解決できなかったため）
- API認証完了: @gachiho_motive / user_id: 26284161057936771
- テスト投稿成功確認
- 自動投稿スケジューラ稼働開始（毎日11:30 + ランダム遅延0〜60分）

**設計（ChatGPT評価 2026-03-19）**:
- 1日1本、14日間、リンクなし/ハッシュタグなし/CTAなし
- 投稿タイプ: 継続肯定5本 / 不安整理5本 / 比較疲れ4本
- X在庫81本を元にThreads版に軽変形（同一文面は使い回さない）
- 安全策: 7日重複禁止 / 類似度チェック / THREADS_ENABLED=false 緊急停止

**判定基準（4/2頃）**:
- 続ける: 数本でも伸びる型がある / 運用負荷が低い / botっぽさなし
- 保留: 届くが反応弱い / 文体調整で改善余地あり
- やめる: 露出弱い / 同質化 / 他媒体より優先度低い

**関連ファイル**: `threads_upload.py`, `threads_auto_post.py`, `threads_candidates.json`, `threads_auth.py`, `threads_token.json`

---

## 自動実行ジョブ稼働状況（2026-03-19更新）

| ジョブ | 時刻 | 状態 |
|-------|------|------|
| YouTube投稿 | 7:00 / 12:00 | 稼働中（A/Bテスト中: 19:00→12:00、3/19〜4/2） |
| Instagram投稿 | 12:00 / 21:00 | 稼働中 |
| X つぶやき | 9:00（+ランダム遅延0〜60分） | 稼働中 |
| Threads投稿 | 11:30（+ランダム遅延0〜60分） | **稼働中（3/19〜、2週間テスト）** |
| X投稿（旧） | 7:30 / 19:30 | 停止中（偽装行為対応で unload済み） |
| note X告知 | 21:30 | 停止中（同上） |
| 分析収集 | 22:00 | 稼働中 |
| 週次分析 | 月曜23:00 | 稼働中 |

---

## 7. X「偽装行為」ラベル対応

**状態**: 2026-03-14に「偽装行為」ラベルが付与。自動投稿を即時停止済み。異議申し立て準備中。

**次のアクション**:
1. ~~異議申し立てを送信~~ → ラベル用の専用フォームが存在しないため断念
2. 手動運用フェーズ（3/14〜3/21）— ChatGPT投稿案あり
   - 1日1投稿（リンクなし、ハッシュタグ0〜1個）— 8日分の投稿文ドラフト済み
   - いいね1日5〜10件、返信1日1〜3件、引用RT週2〜3件
   - プロフィール自己紹介を改善案Aに変更
   - 固定ポストをリンク集→自己紹介型に変更
3. ラベル解除を確認（警告画面消失・検索制限解消）
4. 自動投稿を段階的に再開
   - まず1日1回だけ（手動も並行）
   - テンプレ5〜10パターン、時刻±30分ランダム
   - note告知の自動化は最後

**関連ファイル**: `auto_publish.py`, `x_upload.py`, `note_x_announce.py`, launchd plist 3件

---

## 8. スライド表示品質の安定化（Codex引き継ぎ）

**状態**: Codexで主要改善完了（2026-03-15引き継ぎ）。描画幅ベース折り返し・意味改行・ブランド表示・写真重複回避など実装済み。/simplify でコード品質改善済み。

**残課題**:
1. data/resolve の意味改行（`_semantic_break_candidates`）が全ケースで十分か → トークン追加余地あり
2. resolve 短縮マップ（`_RESOLVE_SLIDE_MAP`）不足 → fallback 時の不自然な略し方
3. 表示テキストの自動検査（はみ出し・途中切れ・意味不明短縮）← ユーザー負荷軽減に最も効く

**関連ファイル**: `slide_gen.py`, `script_gen.py`

---

## 9. スライド・動画品質改善（2026-03-15）

**状態**: 全修正完了・全動画再生成完了

**経緯**:

1. シーン最低表示時間
   - 3/15 7:00投稿「コロナで売った人、1800万円の機会損失」で empathy が0.7秒。テキストが読めない
   - 修正: `video_gen.py` に `MIN_SCENE_SEC = 2.0`（モジュールレベル定数）を追加
   - 115/115本の動画再生成完了

2. 写真素材拡充
   - 4本中3本が同じ hook 画像。全5カテゴリを15枚→30枚に拡充
   - `PHOTO_HISTORY_KEEP` を6→12に変更
   - 117本のスライド+動画再生成完了

3. テキスト折り返し修正
   - `_wrap_text_lines` の `break_long_words=False` で日本語が折り返されない致命的バグを修正
   - Unicode PUA文字プレースホルダ + `break_long_words=True` に変更
   - 117本のスライド+動画再生成完了

4. closing slide_text 修正
   - ループ再生テキストがそのままスライドに載り、長すぎて意味不明だった
   - 16本の closing slide_text を短い CTA（「同じ人いる？」「続けてますか」）に修正
   - 16本のスライド+動画再生成完了

5. empathy slide_text 修正（音声 < テキスト問題）
   - 音声「あなたも。」なのにスライドに「あなたも、あの時売り…」等の読まれないテキスト → 不快
   - ルール: 音声 ≥ テキスト = OK、音声 < テキスト = NG（AGENTS.md に追記済み）
   - 20本の empathy slide_text を音声テキストに揃えて再生成完了

6. 読み辞書追加
   - 「9割退場」「引力」「1800万円」等の読みを追加
   - 「レバレッジ2倍が気になりだした夜」を音声再生成+YouTube再投稿済み

7. コード品質改善（/simplify）
   - `note_publish_additional.py` の旧関数名 import を修正（ImportError防止）
   - `_get_note_articles_from_sheet` の重複を `note_image_replace.py` からの import に変更（50行削減）

**関連ファイル**: `video_gen.py`, `slide_gen.py`, `voice_gen.py`, `note_publish.py`, `note_publish_additional.py`, `AGENTS.md`

---

## 優先度まとめ

| 優先度 | タスク | 状態 |
|--------|--------|------|
| 高 | X「偽装行為」ラベル対応 | 手動運用で信頼回復中（3/14〜3/21） |
| 高 | Cランク57本の整理 | 方針決定済み、38本破棄+19本再査定を実施する |
| 低 | TikTok | API自動投稿は断念、手動投稿で運用 |
| 中 | 新旧A/Bテスト観察 | キュー配置済み、3/28頃判定 |
| 中 | スライド表示品質の安定化 | 主要改善済み、自動検査が残課題 |
| 低 | 長尺動画 3本目 | 2本目の反応待ち（保留） |
| 低 | 英語版チャンネル | 検討・準備済み |
| 済 | note 記事フォーマット統一 | 全32本完了（2026-03-15） |
| 済 | スライド・動画品質改善 | 最低表示2秒+写真30枚+折り返し修正+slide_text修正+読み辞書、全再生成完了 |
| 済 | closingノイズ修正+全動画再生成 | 124/124本完了 |
| 済 | 読み辞書拡充 | 387エントリ |
| 済 | 新プロンプト先行投入 | 14本生成+キュー配置 |
| 済 | 投稿タイミング最適化 | launchd分離済み |
