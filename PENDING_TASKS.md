...
- 2026-03-08: 長尺背景の見せ方を修正。
 ...
 - 2026-03-08: 動き方式の比較テスト実施。`test_motion.py` で static / scale→crop / zoompan の3方式を5秒クリップで比較。ChatGPTレビューの結論:
   - **zoompan は不採用**（揺れの原因は毎フレーム再サンプリング＋整数除算）
   - **scale→crop（iMovie Ken Burns 方式）を採用**
   - ズーム量は 100→103.5% に固定、平行移動はほぼなし（中央基準）
   - ロールごとにズーム方向を固定: 内面系（hook/overview/why_painful/data）= zoom in、余韻系（interpret/action/closing）= zoom out
 - 2026-03-08: `long_video_builder.py` の `_make_video_clip` を zoompan → scale→crop に全面書き換え。`ROLE_ZOOM_DIR` でロールごとのズーム方向を定義。`UPSCALE=1.5` / `ZOOM_RATIO=0.035` をパラメータ化。動画の再生成はまだしていない。

### 残課題
- 次回は `long_video_builder.py` を実行してレビュー用動画を再生成し、scale→crop 方式の揺れ有無を本編尺で最終確認する
- テキストは固定のまま維持する。「背景のみ動かす」は必須条件
...
### 再開時の最初の一手
`long_video_builder.py` を実行してレビュー用動画を再生成し、scale→crop 方式で揺れが解消されたか本編尺で確認する。問題なければ背景画像の差し替えに進む。
