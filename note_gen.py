...
- 1段落は2文を基本、最大3文まで。理由・具体例・結論を1段落に詰め込まない
...
- 段落間の空行は1つだけ（連続空行を入れない。スマホで余白が広がりすぎる）
- 区切り線（---）は使わない（noteのレンダリングで余白が大きくなりすぎるため。見出しだけで区切る）
- 1段落は2文を基本とし、3文まで（スマホで5行を超えない）
