...
# 途中切れしやすい末尾
_DANGLING_SLIDE_ENDING = re.compile(r"(続|思|分か|わか|増え|続け|慣れ|止ま|消え|残っ|効くのは)$")
_SYMBOL_ENDING = re.compile(r"[-−+＋/／]$")
...
def _lint_slide_text(slide_text: str, role: str) -> list[dict]:
    ...
    if _DANGLING_SLIDE_ENDING.search(slide_text):
        issues.append(_issue("error", role, "dangling_slide",
                             f"slide_text が途中で切れています: 「{slide_text}」"))

    if _SYMBOL_ENDING.search(slide_text):
        issues.append(_issue("error", role, "symbol_ending",
                             f"slide_text が記号で終わっています: 「{slide_text}」"))
...
def _lint_data(text: str, slide_text: str, theme_name: str) -> list[dict]:
    ...
    if any(pattern in text for pattern in _DATA_RESOLVE_PATTERNS):
        issues.append(_issue("error", "data", "resolve_like_data",
                             f"dataが結論文に寄りすぎています: 「{text[:30]}」"))

    # 感情テーマ以外では数字が必要
    if theme_name not in _EMOTION_THEMES:
        has_non_numeric_safe_pattern = any(pattern in text for pattern in _NON_NUMERIC_DATA_PATTERNS)
        if not _RE_NUMBER.search(text) and not has_non_numeric_safe_pattern:
