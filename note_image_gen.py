...
def _record_photo(image_name: str, photo_path: pathlib.Path):
    """生成画像と使用写真の対応をJSONに記録する。"""
    log: dict = {}
    if _IMAGE_PHOTO_LOG.exists():
        try:
            log = json.loads(_IMAGE_PHOTO_LOG.read_text(encoding="utf-8"))
        except (json.JSONDecodeError, OSError):
            pass
    log[image_name] = str(photo_path)
    _IMAGE_PHOTO_LOG.write_text(
        json.dumps(log, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )


def get_photo_for_image(image_name: str) -> Optional[pathlib.Path]:
    """記録から画像に使用された写真パスを取得する。"""
    if not _IMAGE_PHOTO_LOG.exists():
        return None
    try:
        log = json.loads(_IMAGE_PHOTO_LOG.read_text(encoding="utf-8"))
        path_str = log.get(image_name)
        if path_str:
...
def generate_note_image(
    ...
    try:
        # --- 背景画像 ---
        bg, photo_path = _load_background(bg_keyword, bg_path)

        # --- 写真パスを記録 ---
        _record_photo(output_path.name, photo_path)
...
def _load_background(
    ...
    if cache_key in _bg_cache:
        return _bg_cache[cache_key].copy(), path
...
def _draw_left_sub_layout(draw: ImageDraw.Draw, title: str, subtitle: str):
    """左寄せ + 下補足型（デフォルト）。タイトル+サブタイトルを垂直中央配置。"""
    ...
    title_gap = 18
    title_total = sum(title_line_heights) + title_gap * (len(lines) - 1) if lines else 0
    ...
    # 全体の高さ → 垂直中央
    total_h = title_total + (sub_gap + sub_h if subtitle else 0)
    y = (NOTE_HEIGHT - total_h) // 2

    # アクセント帯（テキスト範囲に合わせる）
    _draw_accent_band(draw, y - 10, y + total_h + 10)
    ...
    for i, line in enumerate(lines):
        ...
        y += title_line_heights[i] + title_gap

    # 補足テキスト
    if subtitle and font_sub:
        sub_y = y - title_gap + sub_gap  # 最後のtitle_gapを戻してsub_gapに置換
        draw.text(
