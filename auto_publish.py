...
def _optimize_title(raw_title: str, meta: dict) -> str:
    ...
    # タイトルに既にサフィックスのキーワードが含まれている場合はサフィックスを変える
    available = [s for s in _TITLE_SUFFIXES if s not in front]
    if not available:
        available = _TITLE_SUFFIXES
    suffix = random.choice(available)

    title = f"{front}｜{suffix} #Shorts"

    # YouTube タイトル上限 100文字
    if len(title) > 100:
        max_front = 100 - len(f"｜{suffix} #Shorts")
        title = f"{front[:max_front]}｜{suffix} #Shorts"
